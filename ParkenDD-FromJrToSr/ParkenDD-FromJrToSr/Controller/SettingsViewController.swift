...
class SettingsViewController: UITableViewController, MFMailComposeViewControllerDelegate {
	...
	override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
		...
		case .sortingOptions:
			...
			for row in 0...4 {
				tableView.cellForRow(at: IndexPath(row: row, section: Sections.sortingOptions.rawValue))?.accessoryType = .none
			}
		...
		case .displayOptions:
            ...
            case 2:
                ...
                if showExperimentalCities {
                    ...
                    let alert = UIAlertController(title: L10n.noteTitle.string, message: L10n.showexperimentalcitiesalert.string, preferredStyle: .alert)
                    alert.addAction(UIAlertAction(title: L10n.cancel.string, style: .cancel, handler: { (action) -> Void in

                    }))
                    alert.addAction(UIAlertAction(title: L10n.activate.string, style: .default, handler: { (action) -> Void in
		...
		case .otherOptions:
			...
			case 0:
				if #available(iOS 9.0, *) {
				    let safariVC = SFSafariViewController(url: URL(string: "http://parkendd.kilian.io/about.html")!)
